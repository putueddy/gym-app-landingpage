...
<div
  class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 lg:gap-20 relative text-base"
>
  <div
  ...
  <div class={"flex flex-col dropShadow overflow-hidden rounded-b-lg bg-white"}>
    <div
      class="rounded-t-xl h-8 sm:h-10 bg-white opacity-60 px-3 flex items-center gap-2"
    >
      {#each [1, 2, 3] as i}
        <div class="rounded-full aspect-square bg-indigo-400 w-2.5 sm:w-3" />
      {/each}
    </div>
